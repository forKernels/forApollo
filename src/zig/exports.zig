...
const gpu_layer = struct {
    ...
    /// Batched EKF predict on the GPU. Routes to forCUDA if a device is present
    /// and the batch is large enough; sets used_gpu=1 on the GPU path, else 0
    /// (caller falls back to the CPU forapollo_ekf_predict path).
...
comptime {
    if (build_opts.gpu) {
        @export(&gpu_layer.ekfPredictBatchGpu, .{
            .name = "forapollo_ekf_predict_batch_gpu",
            .linkage = .strong,
        });
    }
}
